    ...
    <div className="min-h-[80vh] flex items-center justify-center py-12">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md px-6">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-2xl font-bold mb-2" style={{ color: 'var(--text-primary)' }}>Welcome back</h1>
        ...
        <div className="card p-8">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <a href="/api/v1/auth/oauth/google" className="btn btn-secondary btn-sm justify-center gap-2 no-underline text-sm">
            ...
            <a href="/api/v1/auth/oauth/github" className="btn btn-secondary btn-sm justify-center gap-2 no-underline text-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Link to="/forgot-password" className="text-xs font-medium text-primary no-underline">Forgot?</Link>
              </div>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: 'var(--text-muted)' }} />
                <input type={show ? 'text' : 'password'} required className="input" style={{ paddingLeft: '2.5rem', paddingRight: '2.5rem' }} placeholder="••••••••" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} />
                <button type="button" onClick={() => setShow(!show)} className="absolute right-3 top-1/2 -translate-y-1/2" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>
        ...
        <p className="text-center text-sm mt-6" style={{ color: 'var(--text-secondary)' }}>
          Don't have an account? <Link to="/register" className="text-primary font-semibold no-underline">Create one</Link>
        </p>
      </motion.div>
    </div>
  );
